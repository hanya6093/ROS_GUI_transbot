#include "../include/qt_ros_demo01/CCtrlDashBoard.hpp"
...
void CCtrlDashBoard::drawIndicator(QPainter *painter, qreal hlafWidth)
{
    ...
    if(m_DashValue>0){
        qreal tSteps = (m_MaxValue - m_MinValue)/m_DashNum; //相乘后的值是分的份数 100份
        qreal angleStep = 1.0*(360.0-m_StartAngle - m_EndAngle) / tSteps; //每一个份数的角度
        double degRotate = angleStep*tSteps*(m_DashValue/100.0)+angleStep;
        // double degRotate = angleStep*(m_DashValue/100.0)+angleStep;
...
{
    QPainter p(this);
    qreal width = qMin((this->width()>>1), (this->height()>>1)); // 取窗口小边

    p.setRenderHints(QPainter::Antialiasing|QPainter::TextAntialiasing);

    drawBackGround(&p, width);   // 外黑盘 内盘 标识

    drawScaleDials(&p, width);  // 刻度盘绘制 基于背景绘制
    switch(m_StyleType)
    {
    case ST_DEFAULT:
//        qDebug()<<"drawIndicator";
        drawIndicator(&p, width);    // 绘制指针(默认函数) 基于背景绘制
        break;
    case ST_ARCBAR:
//        qDebug()<<"drawIndicatorBar";
